...
interface Job {
    ...
    name: string;
    filename: string;
    status: 'processing' | 'queued' | 'ready' | 'extracting' | 'reconstructing' | 'rigging';
    progress: number;
    checkStatus: string;
    category?: string;
    region?: string;
...
export function AdminDashboard({ onLogout }: AdminDashboardProps) {
    ...
    const handleStartGenerating = () => {
        ...
        const originalVideoUrl = URL.createObjectURL(pipelineForm.file);
        const newJob: Job = {
            id: jobId,
        ...
        setTimeout(() => {
            setJobs(prev => prev.map(j => j.id === jobId ? { ...j, status: 'extracting', checkStatus: 'MediaPipe: Extracting' } : j));

            const stages: ('extracting' | 'reconstructing' | 'rigging' | 'ready')[] = ['extracting', 'reconstructing', 'rigging', 'ready'];
            ...
            const interval = setInterval(() => {
                setJobs(prev => {
                    ...
                    if (currentJob && currentJob.status !== 'ready') {
                        let nextProgress = currentJob.progress + 2;
                        let nextStatus = currentJob.status;
                        let nextCheck = currentJob.checkStatus;

                        if (nextProgress >= 100) {
                            currentStageIdx++;
                            if (currentStageIdx < stages.length - 1) {
                                nextProgress = 0;
                                nextStatus = stages[currentStageIdx];
                                nextCheck = `Pipeline: ${nextStatus.charAt(0).toUpperCase() + nextStatus.slice(1)}...`;
                            } else {
                                nextProgress = 100;
                                nextStatus = 'ready';
                                nextCheck = 'Safe Check: Verified';
    ...
    const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        if (e.target.files?.[0]) {
            ...
            setPipelineForm({ ...pipelineForm, file });
        }
    };
                                                ...
                                                <tr key={task.id}>
                                                    ...
                                                    <td className="admin-table-cell--center">
                                                        <div style={{ display: 'flex', justifyContent: 'center' }}>
                                                            <div className={`admin-status-badge ${task.status === 'Finished'
                            ...
                            <div className="sign-converter-page">
                                ...
                                <div className="sign-converter-header">
                                    <h2 className="sign-converter-title">
                                        Sign Converter: <span>{currentJob?.name || 'No Job Selected'}</span>
                                    </h2>
                                    <button className="sign-converter-back-btn" onClick={() => setActiveTab('pipeline')}>
                                        ...
                                        <div className="sign-converter-video-grid">
                                                ...
                                                <div className="sign-converter-3d-content">
                                                 ...
                                                 <model-viewer
                                                        ...
                                                        <div className="sign-converter-3d-processing">
                                                            <div className="admin-status-badge admin-status-badge--warning">
                                                                <CircleDashed className="animate-spin" size={24} />
                                                                <span>{currentJob?.checkStatus || 'Processing...'}</span>
                                                            </div>
                                                            <div className="sign-converter-progress-bar" style={{ width: '80%', marginTop: '20px' }}>
                                                                <div className="sign-converter-progress-fill" style={{ width: `${currentJob?.progress || 0}%` }}></div>
                                                            </div>
                                                        </div>
                                                    )}
                                            ...
                                            <div className="sign-converter-original-card">
                                                ...
                                                <div className="sign-converter-original-content">
                                                    {currentJob?.originalVideoUrl ? (
                                                        <video 
                                                            src={currentJob.originalVideoUrl} 
                                                            className="sign-converter-original-video"
                                                            controls
                                                        />
                                                    ) : (
                                                        <div className="sign-converter-original-placeholder">
                                                            <Video size={48} />
                                                ...
                                                <div className="sign-converter-spoken-text">
                                                    ...
                                                    <p className="sign-converter-spoken-quote">"{currentJob?.name || 'Unknown Sign'}"</p>
                                                </div>
                                            </div>
                                        </div>
